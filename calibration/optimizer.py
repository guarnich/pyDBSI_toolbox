...
def optimize_hyperparameters(bvals, bvecs, snr, n_mc=1000):
    ...
    print("-" * 95)
    # Header corretto: stringhe allineate a sinistra (<) con larghezza fissa
    print(f"{'Bases':<6} | {'Lambda':<8} | {'Est(Avg)':<10} | {'Truth':<8} | {'MAE':<8} | {'MSE':<8} | {'Bias':<8}")
    print("-" * 95)
    ...
    for n_iso in bases_grid:
        ...
        for reg in lambdas_grid:
            ...
            print(f"{n_iso:<6} | {reg:<8.2f} | {f_res:<10.4f} | {gt_cell:<8.4f} | {mae:<8.4f} | {mse:<8.4f} | {bias:+8.4f}")            
