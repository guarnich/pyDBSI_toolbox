...
def fit_voxels_pure(data, coords, A, AtA, At, bvals, bvecs,
                    fiber_dirs, iso_grid, reg, enable_step2, fiber_threshold, out):
    """
    ...
    for idx in prange(n_voxels):
        ...
        if f_fib > fiber_threshold:
            ...
            # Optional grid search refinement
            if enable_step2:
...
class DBSI_Fused:
    ...
    def fit(self, data, bvals, bvecs, mask, run_calibration=True):
        ...
        print(f"   AD/RD estimated: {n_fitted:,} / {n_voxels:,} voxels ({pct:.1f}%)")
        print("\n" + "="*70 + "\n")
